...
//     order of distance from the entity that called this function.
func nearestNEntitiesFunc(me *game.Entity) lua.GoFunction {
  ...
  return func(L *lua.State) int {
    n := L.GetTop()
    if n != 2 {
      errstr := fmt.Sprintf("nearestNEntities expects exactly 2 parameters, got %d.", n)
    ...
    kind := L.ToString(1)
    if !valid_kinds[kind] {
      errstr := fmt.Sprintf("nearestNEntities expects kind in the set ['intruder' 'denizen' 'servitor' 'master' 'minion'], got %s.", kind)
      base.Warn().Printf(errstr)
      L.PushString(errstr)
      L.Error()
      return 0
