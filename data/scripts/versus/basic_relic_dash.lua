...
function Init(data)
  ...
  Script.LoadHouse("vs_Relic_Dash")
...
function RoundStart(intruders, round)
  if round == 1 then
    if intruders then
      ...
      Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Opening_Denizens.json")
      denizensSetup()
    end
...
function OnAction(intruders, round, exec)
  ...
  if exec.Action.Type == "Basic Attack" then
    if exec.Target.Name == MasterName and exec.Target.Hp <= 0 then
      ...
      Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Victory_Intruders.json")
    end
  end
  ...
  if exec.Ent.Name == MasterName and GetDistanceBetweenEnts(exec.Ent, Relic) <= 3 and not bCountdownTriggered then
     ...
     Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Countdown_Started_Denizens.json")
  end  
end
...
function RoundEnd(intruders, round)
  ...
  if nCountdown == 0 then
    ...
    Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Victory_Denizens.json")
  end
  ...
  if store.side == "Humans" then
    ...
    if intruders then
      if bCountdownTriggered then
        Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Turns_Remaining_Denizens.json")
      else
        Script.DialogBox("ui/dialog/Relic_Dash/pass_to_denizens.json")
      end
    else
      if not bIntruderIntroDone then
        bIntruderIntroDone = true
        Script.DialogBox("ui/dialog/Relic_Dash/pass_to_intruders.json")
        Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Opening_Intruders.json")
      ...
      if bCountdownTriggered and not bShowedIntruderTimerMessage and not bSkipOtherChecks then
        ...
        Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Countdown_Started_Intruder.json")
        bSkipOtherChecks = true
      end

      if bCountdownTriggered and not bSkipOtherChecks then  --timer is triggered and we've already intro'd it
        Script.DialogBox("ui/dialog/Relic_Dash/Relic_Dash_Turns_Remaining_Intruders.json")
        bSkipOtherChecks = true
      end
